...
{
  // The variable name with $ is a special variable, it means execute the backend script function or execute a process.
  // The value name with @ is a special variable, it means get the data from the backend script, otherwise, it will be executed a process
  "$pets": "@PetList",

  // Call the backend script function, Get the data.
  "$ping": "utils.app.ping",
...
// pet/[id]/[id].json
{
  "$pet": "@Pet" // Call the backend script function, Get the data
}
```

After creating the backend script files, the directory structure should be:
...
In the backend script, we use the `Process` function to call the model.\* process. The `withs` option is used to join the category table and get the category name directly.

**_We should modify the Model DSL to support the join query. It should be done when creating the model. Here demonstrates how to modify the model DSL, and maintain the application._**
